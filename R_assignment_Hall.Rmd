...
- fang_et_al genotypes has 2782 rows and 986 columns
- All columns contain values recognized as character type
- The file has the following structure: Sample_ID, JG_OTU, Group, and then genoytpe data for each SNP_ID
...
- fang_et_al_genotypes has 915 rows and 15 columns
- The columns contain values recognized as character or numeric type
- The columns we need (SNP_ID, Chromosome, Position) are all of type character, even though one might expect that Chromosome and Position would be numeric 
...
#Plot the data
maize_genotypes_long <- maize_genotypes_long %>% mutate(zygosity = fct_relevel(zygosity, 'missing','homozygous','heterozygous'))#'missing' is plotted on top
...
teosinte_genotypes_long <- teosinte_genotypes_long %>% mutate(zygosity = fct_relevel(zygosity, 'missing','homozygous','heterozygous'))#'missing' is plotted on top
